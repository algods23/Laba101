        ...
        <form id="order-form" class="form">
          ...
          <fieldset class="check-grid">
            ...
            ${o.length?o.map(m=>`<div class="qty-card addon-quantity" data-qty-card="addonQty-${m.id}">
              <span><strong>${v(J(m.name))}</strong><small>${N(m.price)} each</small></span>
              <div class="qty-control">
      ...
      <div class="receipt-modal" role="dialog" aria-modal="true" aria-labelledby="receipt-title">
        ...
        <div class="receipt" id="receipt-print-area">
          ...
          <div class="summary-list receipt-lines">
            ...
            ${e.extras.length?`<div><span>Extra services</span><strong>${e.extras.map(i=>`${v(J(i.name))} x${Number(i.quantity??1)} (${N(Number(i.total??i.price))})`).join(", ")}</strong></div>`:""}
        ...
        ${l.selectedTypes.has("summary")?`
          <article>
            ...
            <div class="summary-cards-grid summary-single-row">
              ...
              <div class="summary-card"><span class="card-label">Cash on Hand</span><strong>${N(Ne(l.salesRows().totalCash,l.disbursementRows().totalDisbursement))}</strong></div>
            </div>
          </article>`:""}
      </section>
    `:""}
...
Your email app will open — please attach the file and send.`)}}}catch(p){alert("Failed: "+String(p))}finally{E&&(E.disabled=!1,E.textContent=g==="export"?"Export Excel":"Send to Email")}};document.querySelector("#export-report")?.addEventListener("click",async()=>{await f("export")}),document.querySelector("#email-report")?.addEventListener("click",async()=>{await f("email")})}function ms(){document.querySelector("#order-queue-filters")?.addEventListener("submit",e=>{e.preventDefault();const t=new FormData(e.currentTarget);c.orderSearch=String(t.get("orderSearch")??"").trim(),c.orderDateFilter=String(t.get("orderDateFilter")??"").trim(),c.orderPaymentFilter=String(t.get("orderPaymentFilter")??"").trim(),S()}),document.querySelector("#order-queue-clear")?.addEventListener("click",()=>{c.orderSearch="",c.orderDateFilter="",c.orderPaymentFilter="",S()}),document.querySelector("#archived-order-filters")?.addEventListener("submit",e=>{e.preventDefault();const t=new FormData(e.currentTarget);c.archivedOrderSearch=String(t.get("archivedOrderSearch")??"").trim(),S()}),document.querySelector("#archived-order-clear")?.addEventListener("click",()=>{c.archivedOrderSearch="",S()})}function vs(){document.querySelector("#customer-search-form")?.addEventListener("submit",e=>{e.preventDefault();const t=new FormData(e.currentTarget);c.customerSearch=String(t.get("customerSearch")??"").trim(),S()}),document.querySelector("#customer-search-clear")?.addEventListener("click",()=>{c.customerSearch="",S()})}function ps(){document.querySelector("#machine-form")?.addEventListener("submit",async e=>{e.preventDefault();const t=new FormData(e.currentTarget);await $a({machineName:String(t.get("machineName")??""),machineType:String(t.get("machineType")),status:String(t.get("status")),branch:String(t.get("branch")??"")}),await S()}),document.querySelector("#subcleaning-form")?.addEventListener("submit",async e=>{e.preventDefault();const t=e.currentTarget.querySelector('button[type="submit"]');t&&(t.disabled=!0,t.textContent="Starting...");const a=new FormData(e.currentTarget),s=a.getAll("machineIds").map(Number);if(!s.length){alert("Please select at least one machine to clean."),t&&(t.disabled=!1,t.textContent="Start Cleaning");return}await Oa({date:String(a.get("date")??""),machineIds:s,cleaningStatus:String(a.get("cleaningStatus")??""),notes:String(a.get("notes")??""),branch:String(a.get("branch")??"")}),await S()}),document.querySelectorAll(".complete-cleaning-btn").forEach(e=>{e.addEventListener("click",async()=>{const t=Number(e.dataset.machineId),a=e.dataset.branch??"";e.disabled=!0,e.textContent="Completing...",await Pa(t,a),await S()})}),document.querySelectorAll(".deactivate-machine-btn, .activate-machine-btn").forEach(e=>{e.addEventListener("click",async()=>{const t=Number(e.dataset.id),a=e.classList.contains("deactivate-machine-btn");await Ra(t,a?"inactive":"available"),await S()})})}function fs(e){const t=document.querySelector("#add-staff-modal"),a=document.querySelector("#open-add-staff-modal"),s=document.querySelector("#close-add-staff-modal"),r=document.querySelector("#staff-form"),n=()=>{r?.reset(),r&&(r.querySelector("[name=id]").value="");const o=document.querySelector("#add-staff-title");o&&(o.textContent="Add staff member"),t?.removeAttribute("hidden")},i=()=>{t?.setAttribute("hidden",""),r?.reset()};a?.addEventListener("click",n),s?.addEventListener("click",i),t?.addEventListener("click",o=>{o.target===t&&i()}),document.querySelectorAll(".edit-staff-btn").forEach(o=>{o.addEventListener("click",()=>{const u=Number(o.dataset.id),l=e.find(d=>d.id===u);if(l&&r){r.querySelector("[name=id]").value=String(l.id),r.querySelector("[name=name]").value=l.name,r.querySelector("[name=email]").value=l.email,r.querySelector("[name=password]").value=l.password,r.querySelector("[name=role]").value=l.role,r.querySelector("[name=branch]").value=l.branch;const d=document.querySelector("#add-staff-title");d&&(d.textContent="Edit staff member"),t?.removeAttribute("hidden")}})}),document.querySelectorAll(".deactivate-staff-btn, .activate-staff-btn").forEach(o=>{o.addEventListener("click",async()=>{const u=Number(o.dataset.id),l=e.find(d=>d.id===u);l&&(await Ve(u,{isActive:l.isActive!==0?0:1}),await S())})}),r?.addEventListener("submit",async o=>{o.preventDefault();const u=document.querySelector("#staff-save-btn");u&&(u.disabled=!0,u.textContent="Saving...");const l=new FormData(r),d=l.get("id")?Number(l.get("id")):void 0,m=String(l.get("name")??"").trim(),y=String(l.get("email")??"").trim(),f=String(l.get("password")??"password")||"password",g=String(l.get("role")),E=String(l.get("branch")??"");if(!m||!y){alert("Name and email are required."),u&&(u.disabled=!1,u.textContent="Save staff member");return}try{d?await Ve(d,{name:m,email:y,password:f,role:g,branch:E}):await ra({name:m,email:y,password:f,role:g,branch:E}),i(),await S()}catch{alert("Failed to save staff. The email may already be in use."),u&&(u.disabled=!1,u.textContent="Save staff member")}})}function ys(){document.querySelector("#settings-form")?.addEventListener("submit",async e=>{e.preventDefault();const t=new FormData(e.currentTarget);await ge("branch",String(t.get("branch")??"Main Store")),await ge("fold_rate",String(t.get("foldRate")??"5")),String(t.get("reportEmail")??"")&&await ge("report_email",String(t.get("reportEmail"))),alert("Settings saved successfully!"),await S()})}await ta();const rt=localStorage.getItem(ve);if(rt)try{const e=JSON.parse(rt);if(e.email&&e.remembered){const t=await dt(e.email,"password")??null;c.currentUser=t}}catch{localStorage.removeItem(ve)}function hs(e,t,a,s){const r=e.filter(f=>f.status==="revolving").reduce((f,g)=>f+g.cashAmount,0),n=t.filter(f=>f.type==="add").reduce((f,g)=>f+g.amount,0),i=t.filter(f=>f.type==="disbursement").reduce((f,g)=>f+g.amount,0),o=r+n-i,u=c.revolvingHistoryFrom||"0000-01-01",l=c.revolvingHistoryTo||"9999-12-31",d=t.filter(f=>{const g=H(f.createdAt);return g>=u&&g<=l}),m=e.map(f=>{const g=ht(f.saleDate,a,s,f.cashAmount),E=f.status==="revolving"?'<span class="ok">Revolving</span>':f.status==="endorsed"?`<span class="warn">Endorsed to ${v(f.endorsedTo)}</span>`:'<span class="meta">Pending</span>',p=f.status!=="revolving"&&f.status!=="endorsed"?`<div class="row-actions">
        ...
        </div>`:"";return[`<strong>${v(se(f.saleDate))}</strong>`,`<strong class="ok">${N(g)}</strong>`,E,f.statusUpdatedAt?v(se(f.statusUpdatedAt)):"-",p]}),y=d.map(f=>[qa(f.createdAt),`<strong>${v(f.revolvingNumber)}</strong>`,v(f.name),`<strong class="${f.type==="disbursement"?"warn":"ok"}">${f.type==="disbursement"?"-":"+"}${N(f.amount)}</strong>`,v(f.category),v(f.description||"-"),`<span class="${f.type==="add"?"ok":"warn"}">${f.type==="add"?"Add Revolving Fund":"Disbursement"}</span>`]);return`
    <section class="grid content full">
